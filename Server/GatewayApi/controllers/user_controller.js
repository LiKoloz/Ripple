...
exports.get_user_by_id = (req, res) => {
    jwt.verify(req.body.token, "secret",async (err, token) => {
        ...
        let request = await fetch("http://217.114.0.104:3000/users/get/" + req.params.id); // NOTE: 
...
exports.get_user_by_email = (req, res) => {
    jwt.verify(req.body.token, "secret",async (err, token) => { 
        ...
        let request = await fetch("http://217.114.0.104:3000/users/get/e/" + req.params.email); // NOTE: 
                                                                    //Изменить что бы подтягивалось из Auth - сервиса
        let user = await request.json();
        res.status(200).json(user);
    })
...
exports.sign_in_default = async (req, res) => {
    ...
    axios.post("http://217.114.0.104:3001/sign_in/default", req.body)
    .then((response) => {
...
exports.sign_up_default = async (req, res) => {
    axios.post("http://217.114.0.104:3001/sign_up/default", req.body)
        .then((response) => {
...
exports.check_token = async (req, res) => {
    console.log(req)
    axios.post("http://217.114.0.104:3001/sign_in/check_token", req.body)
        .then((response) => {
            console.log('Responce data \n' + response.data);
